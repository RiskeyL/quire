...
Task-oriented usage for Quire. For install and a first run, see the [README](../README.md).

- [How it works](#how-it-works)
- [Build a manual from a manifest](#build-a-manual-from-a-manifest)
- [Assemble an API reference from OpenAPI specs](#assemble-an-api-reference-from-openapi-specs)
- [Brand the PDF and Word output](#brand-the-pdf-and-word-output)
- [Customize the cover](#customize-the-cover)
- [Link between documents](#link-between-documents)
- [Reuse options with a config file](#reuse-options-with-a-config-file)
- [Scaffold a manifest](#scaffold-a-manifest)
- [Use the theme designer](#use-the-theme-designer)
- [All `convert` options](#all-convert-options)
- [Examples](#examples)
- [Notes and limitations](#notes-and-limitations)
- [Develop Quire](#develop-quire)
...
## Build a manual from a manifest

A manifest is a YAML list that defines what goes into the document and in what order. Three entry types nest freely:
...
## Assemble an API reference from OpenAPI specs

List the specs as `openapi` entries, each its own chapter:
...
image:
  ...
  maxWidth: "100%"      # cap how wide a large image renders in the column; lower (e.g. "85%") to shrink wide screenshots
...
## Reuse options with a config file
...
| `-m, --manifest <file>` | Resolve page order and section hierarchy from a manifest. See [Build a manual from a manifest](#build-a-manual-from-a-manifest). |
| `--title <title>` | Document title, shown on the cover and in the running header. Defaults to the manifest filename. |
| `--theme <file>` | Brand-token YAML (colors, fonts, page size, logo). See [Brand the PDF and Word output](#brand-the-pdf-and-word-output). |
...
| `--base-url <url>` | Published-site base (e.g. `https://docs.dify.ai`). Rewrites links to pages outside the bundle into live external URLs. See [Link between documents](#link-between-documents). |
...
| `-c, --config <file>` | Run-config YAML supplying defaults. CLI flags override it. See [Reuse options with a config file](#reuse-options-with-a-config-file). |
...
Work from source by cloning and building:
...
`npm install` runs the build automatically (via the `prepare` script), so `dist/` is ready afterward. Without `npm link`, run the tool as `node dist/cli.js …`.
